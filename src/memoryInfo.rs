fn print_memory_info(system_table: &mut SystemTable<Boot>) {
    ...
    // print the memory leyout
    println!("Memory map:");
    println!(
        "{:16} {:16} {:8} {}",
        "Physical Addr", "Virtual Addr", "Num Pages", "Size", "Type"
    );
    ...
    for descriptor in memory_map.entries() {
        ...
        print!(
            "{:016X} {:016X} {:12} ",
            descriptor.phys_start, descriptor.virt_start, descriptor.page_count,
        );
    ...
    print_pointer_section(buf_ptr, &memoty_map);

    let heap_buf = system_table
        .boot_services()
        .allocate_pool(MemotyType::LOADER_DATA, 1024)
        .unwrap();
